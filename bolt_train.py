...
def main():
    ...
    print("Downloading starting pretrained weights...")
    os.makedirs("checkpoints", exist_ok=True)
    if not os.path.exists("checkpoints/CroCo_V2_ViTLarge_BaseDecoder.pth"):
        subprocess.check_call([
            "wget", "-nc", "https://download.europe.naverlabs.com/ComputerVision/CroCo/CroCo_V2_ViTLarge_BaseDecoder.pth", 
            "-P", "checkpoints/"
        ])
    ...
    # Stage 1: 224 linear
    ###################################################################
    stage1_output = os.path.join(base_output_dir, "stage1_224")
    
    # In multi-node, we only want the master to create dirs
    if node_rank == "0":
        os.makedirs(stage1_output, exist_ok=True)
    
    train_dataset_str_224 = (
        f" + 100_000 @ Habitat(1_000_000, ROOT='{data_dir}/habitat_processed', split='train', aug_crop=16, resolution=224, transform=ColorJitter)"
        f" + 100_000 @ BlendedMVS(ROOT='{data_dir}/blendedmvs_processed', split='train', aug_crop=16, resolution=224, transform=ColorJitter)"
        f" + 100_000 @ MegaDepth(ROOT='{data_dir}/megadepth_processed', split='train', aug_crop=16, resolution=224, transform=ColorJitter)"
        f" + 100_000 @ ARKitScenes(ROOT='{data_dir}/arkitscenes_processed', split='train', aug_crop=256, resolution=224, transform=ColorJitter)"
        f" + 100_000 @ Co3d(ROOT='{data_dir}/co3d_processed', split='train', aug_crop=16, mask_bg='rand', resolution=224, transform=ColorJitter)"
        f" + 100_000 @ StaticThings3D(ROOT='{data_dir}/staticthings3d_processed', aug_crop=256, mask_bg='rand', resolution=224, transform=ColorJitter)"
        f" + 100_000 @ ScanNetpp(ROOT='{data_dir}/scannetpp_processed', split='train', aug_crop=256, resolution=224, transform=ColorJitter) "
        f" + 100_000 @ Waymo(ROOT='{data_dir}/waymo_processed', split='train', aug_crop=16, resolution=224, transform=ColorJitter) "
        f" + 100_000 @ WildRGBD(ROOT='{data_dir}/wildrgbd_processed', split='train', aug_crop=16, resolution=224, transform=ColorJitter) "
    )

    test_dataset_str_224 = (
        f" 1_000 @ Habitat(10_000, ROOT='{data_dir}/habitat_processed', split='val', resolution=224, seed=777)"
        f" + 1_000 @ BlendedMVS(ROOT='{data_dir}/blendedmvs_processed', split='val', resolution=224, seed=777)"
        f" + 1_000 @ MegaDepth(ROOT='{data_dir}/megadepth_processed', split='val', resolution=224, seed=777)"
        f" + 1_000 @ Co3d(ROOT='{data_dir}/co3d_processed', split='test', mask_bg='rand', resolution=224, seed=777) "
    )

    cmd_stage1 = torchrun_prefix + [
        f"--train_dataset={train_dataset_str_224}",
        f"--test_dataset={test_dataset_str_224}",
        ...
        "--epochs=100",
        "--batch_size=16",
        f"--accum_iter={stage1_accum}",
        "--save_freq=5",
        "--keep_freq=10",
        "--eval_freq=1",
        f"--output_dir={stage1_output}"
    ]

    run_stage(1, cmd_stage1)
    ...
    stage1_best_ckpt = os.path.join(stage1_output, "checkpoint-best.pth")
